...
select name
from Highschooler, Friend on Highschooler.ID = Friend.ID1
...
from Highschooler H1
where ID not in
(select ID1 from Friend, Highschooler H2
...
(select ID2 from Friend where ID1 = H1.ID) and (H1.ID = F1.ID1 and H3.ID = F1.ID2) and (H2.ID = F2.ID1 and H3.ID = F2.ID2);
...
select name, grade
from Highschooler join Likes on Highschooler.ID = Likes.ID2
group by name
having count(name) > 1;